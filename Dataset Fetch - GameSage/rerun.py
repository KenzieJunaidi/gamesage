...
# Load backup dataset (Only this file will be updated)
...
# Filter out already processed games (from backup only)
app_list = [app for app in app_list if str(app["appid"]) not in backup_dataset]
...
for i in range(0, len(app_list), BATCH_SIZE):
    ...
    for app in batch:
        ...
        if game_data:
            ...
            # Fetch review score description
            ...
            # Add new game to backup dataset
            backup_dataset[str(appID)] = game_entry  
            
            print(f"Added missing game: {name} (AppID: {appID})")

            # Save last processed AppID
            with open(LAST_SAVED_FILE, "w") as f:
                f.write(str(appID))

    # Save progress every batch (Backup only)
    with open(BACKUP_FILE, "w", encoding="utf-8") as f:
        json.dump(backup_dataset, f, indent=4)

    print(f"Saved batch {i // BATCH_SIZE + 1}, last AppID: {appID}")
    time.sleep(SLEEP_TIME)

print("All missing games fetched and saved successfully!")
